...
使用固定边界条件、不随积分改变；每步后做空间平滑，
并在最终结果上做三点时间平滑 + 再空间平滑，
绘制初始场与平滑后预报场的 500 hPa 重力位势高度
"""
    ...
    for i in range(1,m1):
        for j in (1,n1-1):
            w[i,j] = (a[i,j]
                      ...
                      +0.25*s*s*(a[i-1,j-1]+a[i-1,j+1]+
                                 a[i+1,j-1]+a[i+1,j+1]-4*a[i,j]))
    for i in (1,m1-1):
        for j in range(1,n1):
            w[i,j] = (a[i,j]
                      +0.5*s*(1-s)*(a[i-1,j]+a[i+1,j]+a[i,j-1]+a[i,j+1]-4*a[i,j])
...
def main():
    ...
    # 初始场 t=0
    u0 = u500.isel(valid_time=0).values
    v0 = v500.isel(valid_time=0).values
    z0 = z500.isel(valid_time=0).values
    ...
    # 记录固定边界
    ua_bt, ua_bb = ua[0,:].copy(), ua[-1,:].copy()
    ua_bl, ua_br = ua[:,0].copy(), ua[:,-1].copy()
    va_bt, va_bb = va[0,:].copy(), va[-1,:].copy()
    va_bl, va_br = va[:,0].copy(), va[:,-1].copy()
    za_bt, za_bb = za[0,:].copy(), za[-1,:].copy()
    za_bl, za_br = za[:,0].copy(), za[:,-1].copy()
    ...
    for k in tqdm(range(6), desc="积分进度"):
        ...
        ub[1:-1,1:-1] = tu[1:-1,1:-1]
        vb[1:-1,1:-1] = tv[1:-1,1:-1]
        zb[1:-1,1:-1] = tz[1:-1,1:-1]
        # 固定边界回置
        ub[0,:], ub[-1,:] = ua_bt, ua_bb; ub[:,0], ub[:,-1] = ua_bl, ua_br
        vb[0,:], vb[-1,:] = va_bt, va_bb; vb[:,0], vb[:,-1] = va_bl, va_br
        zb[0,:], zb[-1,:] = za_bt, za_bb; zb[:,0], zb[:,-1] = za_bl, za_br
        # 空间平滑
        ub = ssbp(ub,s,m,n); vb = ssbp(vb,s,m,n); zb = ssbp(zb,s,m,n)

        # 半步修正
        tu,tv,tz = ti(ua,va,za, ub,vb,zb, rm,f,d,dt,zo,m,n)
        ua[1:-1,1:-1] = tu[1:-1,1:-1]
        va[1:-1,1:-1] = tv[1:-1,1:-1]
        za[1:-1,1:-1] = tz[1:-1,1:-1]
        # 固定边界回置
        ua[0,:], ua[-1,:] = ua_bt, ua_bb; ua[:,0], ua[:,-1] = ua_bl, ua_br
        va[0,:], va[-1,:] = va_bt, va_bb; va[:,0], va[:,-1] = va_bl, va_br
        za[0,:], za[-1,:] = za_bt, za_bb; za[:,0], za[:,-1] = za_bl, za_br
        # 空间平滑
    ...
    plot_field(ax2, zb_ts, lmda, phai, '(b) 平滑后 12h 场')
