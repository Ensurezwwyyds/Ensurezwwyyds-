...
# 计算参考长度，使用原点位置(100°E，50°N)
l_ref = a * np.cos(np.deg2rad(50)) * np.tan(np.deg2rad(phy0))
...
df_m.to_excel('map_scale_exp1.xlsx', index=False, header=False)
df_f.to_excel('coriolis_force_exp1.xlsx', index=False, header=False)
